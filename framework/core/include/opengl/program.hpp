...
public:
    ...
    //
    // Befare of variants with names instead of direct locations. They incur performance hit due to lookup of the location.
    //
...
#define UNIFORM_FUNCTIONS_TYPE(T, GL_T)                                                                                          \
    void uniform(uint32_t location, T v0) const { glProgramUniform1##GL_T(program, location, v0); };                             \
    void uniform(uint32_t location, T v0, T v1) const { glProgramUniform2##GL_T(program, location, v0, v1); };                   \
    void uniform(uint32_t location, T v0, T v1, T v2) const { glProgramUniform3##GL_T(program, location, v0, v1, v2); };         \
    void uniform(uint32_t location, T v0, T v1, T v2, T v3) const {                                                              \
        glProgramUniform4##GL_T(program, location, v0, v1, v2, v3);                                                              \
    };                                                                                                                           \
                                                                                                                                 \
    void uniform(std::string_view name, T v0) const { uniform(get_uniform_location(name), v0); };                                \
    void uniform(std::string_view name, T v0, T v1) const { uniform(get_uniform_location(name), v0, v1); };                      \
    void uniform(std::string_view name, T v0, T v1, T v2) const { uniform(get_uniform_location(name), v0, v1, v2); };            \
    void uniform(std::string_view name, T v0, T v1, T v2, T v3) const { uniform(get_uniform_location(name), v0, v1, v2, v3); };  \
                                                                                                                                 \
    void uniform(uint32_t location, glm::tvec2<T> v) const { glProgramUniform2##GL_T(program, location, v.x, v.y); };            \
    void uniform(uint32_t location, glm::tvec3<T> v) const { glProgramUniform3##GL_T(program, location, v.x, v.y, v.z); };       \
    void uniform(uint32_t location, glm::tvec4<T> v) const {                                                                     \
        glProgramUniform4##GL_T(program, location, v.x, v.y, v.z, v.w);                                                          \
    };                                                                                                                           \
                                                                                                                                 \
    void uniform(std::string_view name, glm::tvec2<T> v) const { uniform(get_uniform_location(name), v.x, v.y); };               \
    void uniform(std::string_view name, glm::tvec3<T> v) const { uniform(get_uniform_location(name), v.x, v.y, v.z); };          \
    void uniform(std::string_view name, glm::tvec4<T> v) const { uniform(get_uniform_location(name), v.x, v.y, v.z, v.w); };     \
                                                                                                                                 \
    void uniform(uint32_t location, std::array<T, 2> v) const { uniform(location, v[0], v[1]); };                                \
    void uniform(uint32_t location, std::array<T, 3> v) const { uniform(location, v[0], v[1], v[2]); }                           \
    void uniform(uint32_t location, std::array<T, 4> v) const { uniform(location, v[0], v[1], v[2], v[3]); }                     \
                                                                                                                                 \
    void uniform(std::string_view name, std::array<T, 2> v) const { uniform(get_uniform_location(name), v[0], v[1]); };          \
    void uniform(std::string_view name, std::array<T, 3> v) const { uniform(get_uniform_location(name), v[0], v[1], v[2]); };    \
    ...
    void uniform(std::string_view name, std::span<T> v) const { uniform(get_uniform_location(name), v); }
...
#define UNIFORM_ARRAY_FUNCTIONS_TYPE(T, GL_T)                                                                                    \
    ...
    void uniform_array(std::string_view name, std::span<T> values) const { uniform_array(get_uniform_location(name), values); }  \
