...
struct PhongLightData {
  ...
  public:
    ...
    PhongLightData()
        : position(glm::vec4(0.0f)), ambient(glm::vec3(0.0f)), padding1(0), diffuse(glm::vec3(0.0f)), padding2(0), specular(glm::vec3(0.0f)), padding3(0), spot_direction(glm::vec3(0.0f)),
          spot_exponent(0), spot_cos_cutoff(0), atten_constant(0), atten_linear(0), atten_quadratic(0) {}
    ...
    PhongLightData(const glm::vec4& position, const glm::vec3& ambient, const glm::vec3& diffuse, const glm::vec3& specular, const glm::vec3& spot_direction, float spot_exponent,
        ...
        : position(position), ambient(ambient), diffuse(diffuse), specular(specular), spot_direction(spot_direction), spot_exponent(spot_exponent), spot_cos_cutoff(spot_cos_cutoff),
          atten_constant(atten_constant), atten_linear(atten_linear), atten_quadratic(atten_quadratic) {}
  ...
  public:
    ...
    /** Factory method for a point light, sets all attributes that are not specified to default values. */
    static PhongLightData CreatePointLight(const glm::vec3& position, const glm::vec3& ambient, const glm::vec3& diffuse, const glm::vec3& specular, float atten_constant, float atten_linear,
                                           float atten_quadratic) {
...
class PhongLightsUBO : public UBO<PhongLightData> {
  ...
  public:
    ...
    /**
     * Constructs a new @link PhongLightsUBO.
     *
     * @param 	lights_count	TRhe maximum number of lights stored in this buffer.
     * @param 	target			The target to which this buffer can be bound.
     */
    PhongLightsUBO(int lights_count = 8, GLenum target = GL_UNIFORM_BUFFER) : UBO<PhongLightData>(GL_DYNAMIC_STORAGE_BIT, target, true /* we initialize the OpenGL buffer ourselves */) {
     ...
     * @return A shallow copy of the buffer object that was moved into this object.
     */
  ...
  public:
     ...
     */
    std::vector<PhongLightData>& get_lights() { return this->data; }

    /**
     * Returns a particular light stored in the buffer on the specified position.
     *
     * @return	The light stored in the buffer on the specified position.
     */
    PhongLightData& get_light(int index) {
        // Fails if the light index is out of range.
